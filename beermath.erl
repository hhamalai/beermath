...
malt_bill(Malts, VolumeLitres, TargetGravity) 
    -> malt_bill2(Malts, litres2gallons(VolumeLitres) * TargetGravity).
malt_bill2(Malts, TotalGravity) 
    -> [{Malt, lbs2kgs(Proportion * TotalGravity / Efficiency / ExtractPotential) }
        || {Malt, ExtractPotential, Efficiency, Proportion} <- Malts].
...
gravity_correction(fahrenheit, Fahrenheit)
    -> 1.313454 - 0.132674 * Fahrenheit + 0.002057793 * math:pow(Fahrenheit, 2) - 0.000002627634 * math:pow(Fahrenheit, 3).
...
ibus(AlphaAcidRating, Grams, Volume, Mins, WortGravity)
    -> aa_util(Mins, WortGravity) * aa_concentration(AlphaAcidRating, Grams, Volume).
